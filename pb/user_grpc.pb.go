...
type UserServiceClient interface {
	...
	AddUserStremBoth(ctx context.Context, opts ...grpc.CallOption) (UserService_AddUserStremBothClient, error)
}
...
func (c *userServiceClient) AddUserStremBoth(ctx context.Context, opts ...grpc.CallOption) (UserService_AddUserStremBothClient, error) {
	stream, err := c.cc.NewStream(ctx, &UserService_ServiceDesc.Streams[2], "/pb.UserService/AddUserStremBoth", opts...)
	if err != nil {
		return nil, err
	}
	x := &userServiceAddUserStremBothClient{stream}
	return x, nil
}

type UserService_AddUserStremBothClient interface {
	Send(*User) error
	Recv() (*UserResultStream, error)
	grpc.ClientStream
}

type userServiceAddUserStremBothClient struct {
	grpc.ClientStream
}

func (x *userServiceAddUserStremBothClient) Send(m *User) error {
	return x.ClientStream.SendMsg(m)
}

func (x *userServiceAddUserStremBothClient) Recv() (*UserResultStream, error) {
	m := new(UserResultStream)
	if err := x.ClientStream.RecvMsg(m); err != nil {
		return nil, err
	}
...
type UserServiceServer interface {
	...
	AddUserStremBoth(UserService_AddUserStremBothServer) error
...
func (UnimplementedUserServiceServer) AddUserStremBoth(UserService_AddUserStremBothServer) error {
	return status.Errorf(codes.Unimplemented, "method AddUserStremBoth not implemented")
}
...
func _UserService_AddUserStremBoth_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(UserServiceServer).AddUserStremBoth(&userServiceAddUserStremBothServer{stream})
}

type UserService_AddUserStremBothServer interface {
	Send(*UserResultStream) error
	Recv() (*User, error)
	grpc.ServerStream
}

type userServiceAddUserStremBothServer struct {
	grpc.ServerStream
}

func (x *userServiceAddUserStremBothServer) Send(m *UserResultStream) error {
	return x.ServerStream.SendMsg(m)
}

func (x *userServiceAddUserStremBothServer) Recv() (*User, error) {
	m := new(User)
	if err := x.ServerStream.RecvMsg(m); err != nil {
		return nil, err
	}
...
var UserService_ServiceDesc = grpc.ServiceDesc{
	...
	Streams: []grpc.StreamDesc{
		...
		{
			StreamName:    "AddUserStremBoth",
			Handler:       _UserService_AddUserStremBoth_Handler,
			ServerStreams: true,
			ClientStreams: true,
		},
	},
